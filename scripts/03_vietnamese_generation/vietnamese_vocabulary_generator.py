...
class VietnameseGeneratorConfig:
    ...
    def __init__(self):
        ...
        self.output_dir = "data/vietnamese_generated"
...
def generate_vietnamese_entry(chinese_word: str, chinese_pinyin: str, chinese_meanings: List[str], 
    ...
    meanings_str = "; ".join(chinese_meanings) if chinese_meanings else "meanings not provided"
    ...
    for attempt in range(config.max_retries):
        try:
            logging.info(f"API request for {chinese_word} (attempt {attempt+1})")
            ...
            if response.status_code == 200:
                ...
                logging.info(f"Response received for {chinese_word} ({len(response_text)} chars)")
                ...
                if response_text.startswith("<think>") and "</think>" in response_text:
                    ...
                    logging.debug(f"Removed thinking section ({len(thinking_text)} chars)")
                ...
                if json_start >= 0 and json_end > json_start:
                    ...
                    try:
                        ...
                        required_fields = ["vietnamese", "syllables", "frequency", "pos", "etymology", "forms"]
                        if all(field in vietnamese_entry for field in required_fields):
                            # Additional validation for nested required fields
                            if (vietnamese_entry.get("etymology", {}).get("origin") and
                                vietnamese_entry.get("forms") and len(vietnamese_entry["forms"]) > 0):
                                
                                form = vietnamese_entry["forms"][0]
                                required_form_fields = ["standard", "transcriptions", "regional_variants", 
                                                      "meanings", "register", "formality_level"]
                                
                                if all(field in form for field in required_form_fields):
                                    logging.info(f"Successfully generated Vietnamese entry for {chinese_word}")
                                    return vietnamese_entry
                                else:
                                    missing = [f for f in required_form_fields if f not in form]
                                    logging.warning(f"Missing required form fields for {chinese_word}: {missing}")
                            else:
                                logging.warning(f"Missing required nested fields for {chinese_word}")
                        else:
                            missing = [f for f in required_fields if f not in vietnamese_entry]
                            logging.warning(f"Missing required fields for {chinese_word}: {missing}")
                    
                    except json.JSONDecodeError as e:
                        logging.error(f"JSON parse error for {chinese_word}: {e}")
                        logging.error(f"Problematic JSON: {json_str[:200]}...")
                
                else:
                    logging.error(f"No valid JSON found in response for {chinese_word}")
            
            else:
                logging.error(f"API error for {chinese_word}: {response.status_code} - {response.text}")
...
def extract_chinese_vocabulary(hsk_file_path: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logging.info(f"Extracted {len(vocabulary_items)} Chinese vocabulary items")
...
def process_hsk_level(level: int, config: VietnameseGeneratorConfig):
    ...
    output_file = os.path.join(config.output_dir, f"vietnamese_raw_{level}.json")
    progress_file = os.path.join(config.output_dir, f"vietnamese_raw_{level}_progress.json")
    
    logging.info(f"Generating Vietnamese vocabulary for {len(chinese_vocabulary)} items from HSK {level}")
    ...
    for i in range(0, len(chinese_vocabulary), config.batch_size):
        ...
        total_batches = (len(chinese_vocabulary) + config.batch_size - 1) // config.batch_size
        
        logging.info(f"Processing batch {batch_num}/{total_batches} (items {i+1}-{min(i+config.batch_size, len(chinese_vocabulary))})")
        ...
        for item in batch:
            ...
            if vietnamese_entry:
                ...
                processed_count += 1
                logging.info(f"Generated entry {processed_count}/{len(chinese_vocabulary)}: {vietnamese_entry.get('vietnamese', 'Unknown')}")
    ...
    logging.info(f"Completed HSK level {level}: Generated {len(vietnamese_vocabulary)} Vietnamese vocabulary entries")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Vietnamese Vocabulary Generator from HSK Data')
    ...
    parser.add_argument('--raw-dir', type=str, default=config.raw_dir, help='Directory with raw HSK JSON files')
    parser.add_argument('--output-dir', type=str, default=config.output_dir, help='Directory for Vietnamese output files')
