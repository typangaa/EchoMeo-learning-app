...
def main():
    ...
    # Navigate to project root from scripts/04_data_analysis/hsk_analysis/
    project_root = os.path.join(script_dir, '..', '..', '..')
    project_root = os.path.abspath(project_root)
    
    # Define data paths
    raw_data_dir = os.path.join(project_root, 'scripts', 'data', 'raw', 'exclusive')
    enriched_data_dir = os.path.join(project_root, 'scripts', 'data', 'enriched', 'vocabulary')
    output_dir = os.path.join(script_dir, 'analysis_output')
    
    print(f"Project root: {project_root}")
    print(f"Raw data directory: {raw_data_dir}")
    print(f"Enriched data directory: {enriched_data_dir}")
    ...
    if not os.path.exists(enriched_data_dir):
        print(f"Warning: Enriched data directory does not exist: {enriched_data_dir}")
        print("Creating directory structure...")
