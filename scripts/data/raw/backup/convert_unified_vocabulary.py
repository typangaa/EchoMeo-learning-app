#!/usr/bin/env python
"""
Unified Vocabulary Converter for Vietnamese-Chinese Learning App

This script:
1. Reads enriched unified vocabulary data
...
Usage:
  python convert_unified_vocabulary.py [--input-dir INPUT_DIR] [--output-file OUTPUT_FILE]
"""
...
def load_vocabulary_data(input_dir: str) -> List[Dict[str, Any]]:
    """
    Load unified vocabulary data from the input directory.
    
    Args:
        input_dir: Directory containing vocabulary data
    ...
    # Try to load from vocabulary directory
    vocab_dir = os.path.join(input_dir, "vocabulary")
    if os.path.exists(vocab_dir):
        for filename in os.listdir(vocab_dir):
            if filename.endswith("_vocabulary.json"):
                try:
    ...
    # If no vocabulary found, try loading from combined directory as fallback
    ...
    # If still no data, try loading from traditional separate directories
    if not all_vocabulary:
        # Load both characters and words
        char_dir = os.path.join(input_dir, "characters")
        if os.path.exists(char_dir):
            for filename in os.listdir(char_dir):
                if filename.endswith("_enriched.json") or filename.endswith("_chars_enriched.json"):
                    try:
        ...
        if os.path.exists(word_dir):
            for filename in os.listdir(word_dir):
                if filename.endswith("_enriched.json") or filename.endswith("_words_enriched.json"):
                    try:
...
def determine_hsk_level(item: Dict[str, Any]) -> int:
    ...
    Args:
        item: Enriched vocabulary data
        
    Returns:
        HSK level (1-7)
    """
    # Check if there's an explicit hsk_level field
...
def convert_to_app_format(vocabulary_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Convert enriched vocabulary data to application format.
    
    Args:
        vocabulary_data: List of enriched vocabulary items
    ...
    for item_data in vocabulary_data:
        # Get vocabulary text
        vocabulary = ""
        if "vocabulary" in item_data:
            vocabulary = item_data["vocabulary"]
        elif "character" in item_data:
            vocabulary = item_data["character"]
        elif "word" in item_data:
            vocabulary = item_data["word"]
        
        # Skip empty or already processed items
        if not vocabulary or vocabulary in processed_items:
            continue
        
        processed_items.add(vocabulary)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Convert unified vocabulary data to application format')
    ...
    logging.info(f"Loading vocabulary data from {args.input_dir}")
    vocabulary_data = load_vocabulary_data(args.input_dir)
    
    # Convert to application format
    logging.info("Converting to application format")
    app_vocab = convert_to_app_format(vocabulary_data)
